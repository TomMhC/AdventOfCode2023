...
namespace AdventOfCode
{
    internal class Program
    {
        ...
        {
            var stream = Assembly.GetExecutingAssembly().GetManifestResourceStream("Challenge_3.Input-Dummy.txt");
            var games = LoadGames(stream);

            var total = games
                .Where(g => g.Sets.All(s => s.Red <= 12 && s.Green <= 13 && s.Blue <= 14))
                .Sum(g => g.Id);
            Console.WriteLine(total);

            stream = Assembly.GetExecutingAssembly().GetManifestResourceStream("Challenge_3.Input.txt");
